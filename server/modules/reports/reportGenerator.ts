...
import { getDb, apontamentos, salas } from '../../common/db';
...
export async function generateExcelReport(projectId: string, edificacao?: string): Promise<Buffer> {
    ...
    if (edificacao && edificacao !== 'Todas') sConditions.push(eq(salas.edificacao, edificacao));
    const rawSalasData = await database.select().from(salas).where(and(...sConditions));
    ...
    if (edificacao && edificacao !== 'Todas') aConditions.push(eq(apontamentos.edificacao, edificacao));
    const apontamentosData = await database.select().from(apontamentos).where(and(...aConditions));
